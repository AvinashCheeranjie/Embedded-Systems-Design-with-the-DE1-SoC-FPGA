module seven_seg_decoder(input [3:0] x, output [6:0] hex_LEDs);
	...
	assign hex_LEDs[0]= (!x[3]&!x[2]&!x[1]&x[0]) | (!x[3]&x[2]&!x[1]&!x[0]) | (x[3]&!x[2]&x[1]&x[0]) | (x[3]&x[2]&!x[1]&!x[0]) | (x[3]&x[2]&!x[1]&x[0]) | (x[3]&x[2]&x[1]&x[0]);
	assign hex_LEDs[1]= (!x[3]&x[2]&!x[1]&x[0]) | (!x[3]&x[2]&x[1]&!x[0]) | (x[3]&x[2]&!x[1]&!x[0]) | (x[3]&x[2]&!x[1]&x[0]) | (x[3]&x[2]&x[1]&x[0]);
	...
	begin
		case (x)
			...
			4'b1010: reg_LEDs[6:2]=5'b00010; // 7'b0001000 char	 A
			4'b1011: reg_LEDs[6:2]=5'b10000; // 7'b1000001 char 	 V
			4'b1100: reg_LEDs[6:2]=5'b10011; // 7'b1001111 char	 I
			4'b1101: reg_LEDs[6:2]=5'b01010; // 7'b0101011 char    n
			4'b1110: reg_LEDs[6:2]=5'b00010; // 7'b0001000 char    A
